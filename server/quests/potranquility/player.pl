...
sub EVENT_CLICKDOOR {
  # Level-only access shortcut is only allowed once Gates of Discord is the active
  # expansion. While PoP (and LoY/LDoN) is current, these zones require full
  # progression flag completion (or pop_alt_access_* back-flag globals).
  my $god_active = quest::is_gates_of_discord_enabled();
  #valor/storms
  if($doorid == 16 || $doorid == 21) {
    if(($god_active && $client->GetLevel() >= $level_for_tier_two) || (defined $qglobals{pop_poj_mavuin} &&  defined $qglobals{pop_poj_tribunal} &&  defined $qglobals{pop_poj_valor_storms})) {
  ...
  #codecay
  if($doorid == 12) {
    if(($god_active && $client->GetLevel() >= $level_for_tier_two) || (defined $qglobals{pop_pod_alder_fuirstel} && defined $qglobals{pop_pod_grimmus_planar_projection} && defined $qglobals{pop_pod_elder_fuirstel}) || (defined $qglobals{pop_alt_access_codecay})) {
  ...
  #potactics
  if($doorid == 24) {
    if(($god_active && $client->GetLevel() >= $level_for_tier_three_five) || (defined $qglobals{pop_poi_behometh_preflag} && defined $qglobals{pop_poi_behometh_flag}) || (defined $qglobals{pop_alt_access_potactics})) {
