...
sub EVENT_KILLED_MERIT {
	...
	quest::set_data("luclin_diaboxivatemariel_" . $account_id, $char_name);
	my $first_key = "first_kill_diaboxivatemariel";
	unless (quest::get_data($first_key) || $client->GetGM()) {
		quest::set_data($first_key, $char_name . "|" . $uguild);
		quest::we(15, "SERVER FIRST! " . $char_name . " <" . $uguild . "> and their group have slain Diabo Xi Va Temariel for the first time on this server!");
	}
}
